...
args.output_dir='/nfs/turbo/coe-hunseok/sshoouri/pascal_finetune/sal/step3_sara_m_three'
...
tasks_loss_fn = {
            'sal': CrossEntropyLoss(balanced=True, ignore_index=255) 
}
...
def evaluate(model, tasks_loss_fn, data_loader, device, epoch, in_domains,
    ...
    for x in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            task_losses = {
                task: tasks_loss_fn[task](preds[task].float(), tasks_dict['sal'])
                for task in preds
            }
...
args.finetune_pretrained='/nfs/turbo/coe-hunseok/sshoouri/pascal_finetune/main_seg/step1_sara_m/checkpoint-best.pth'
...
saved = torch.load('/nfs/turbo/coe-hunseok/sshoouri/pascal_finetune/sal/step2_sara_m_two/checkpoint-last-info.pt', map_location='cpu')
checkpoint_normal={}
...
for epoch in range(args.start_epoch, args.epochs):
    ...
    if epoch % args.eval_freq == 0:
            ...
            if val_stats['maxF'] > 0.8528:
                ...
                log_stats = {**{f'train/{k}': v for k, v in train_stats.items()},
                         **{f'best_val/{k}': v for k, v in val_stats.items()},
                         'epoch': epoch,
                         'n_parameters': n_parameters}
    else:
                log_stats = {**{f'train/{k}': v for k, v in train_stats.items()},
