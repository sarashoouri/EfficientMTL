...
args.output_dir='./pascal_finetune/human_parts/step3_sara_m_three'
...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(self, out, label, reduction='mean'):
        ...
        if reduction == 'mean':
            ...
            return loss
tasks_loss_fn = {
            'human_parts': CrossEntropyLoss(ignore_index=255)
}
...
def evaluate(model, tasks_loss_fn, data_loader, device, epoch, in_domains,
    ...
    pred_images = None
    gt_images = None
    performance_meter=HumanPartsMeter('PASCALContext', ignore_idx=255)
    for x in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            task_losses = {
                task: tasks_loss_fn[task](preds[task].float(), tasks_dict['human_parts'])
                for task in preds
            }
        ...
        output = preds['human_parts'].permute(0, 2, 3, 1)
        _, output = torch.max(output, dim=3)
        targets = tasks_dict['human_parts'] 
    ...
    metric_logger.update(mIoU=res['mIoU'])
...
input_adapters_p= {
        'rgb': partial(PatchedInputAdapter, num_channels=3)(
            stride_level=1,
            patch_size_full=args.patch_size,
            image_size=args.input_size,
            learnable_pos_emb=args.learnable_pos_emb,
...
output_adapters_p= {
        'semseg': adapters_dict['convnext'](
            num_classes=21,
            embed_dim=3072, patch_size=args.patch_size,
        ),
}
model_depth_main = create_model(
        args.model,
        input_adapters=input_adapters_p,
        output_adapters=output_adapters_p,
        drop_path_rate=args.drop_path_encoder,
)
args.finetune_pretrained='./pascal_finetune/main_seg/step1/checkpoint-best.pth'

checkpoint = torch.load(args.finetune_pretrained, map_location='cpu')
checkpoint_model = checkpoint['model']
interpolate_pos_embed_multimae(model_depth_main, checkpoint_model)


msg = model_depth_main.load_state_dict(checkpoint_model, strict=False)
print(msg)

saved = torch.load('./pascal_finetune/human_parts/step2/checkpoint-last-info.pt', map_location='cpu')
checkpoint_normal={}
for n in saved["model"]:
    m=n.replace("module.", "")
    checkpoint_normal[m]=saved["model"][n]
interpolate_pos_embed_multimae(model, checkpoint_normal)    
model.load_state_dict(checkpoint_normal)

# calculate current sparsity
nonzero = 0
total = 0
state_dict = model.state_dict()
state_dict_pretrained = model_depth_main.state_dict()
for n in state_dict:
    if "output" in n: 
        continue 
    extra = state_dict[n] - state_dict_pretrained[n]
    nonzero += torch.sum(extra != 0).item()
    total += extra.numel()
print("=====> Initial sparsity: %.4lf" % (100.0 * (1-nonzero / total)))
print("total=", total, "nonzero=", nonzero)
model_depth_main.to(device)
model.to(device)
...
layer_name_list = []
for i in range(0,12): 
            layer_name_list.append("encoder." + str(i) + ".norm1")
            layer_name_list.append("encoder." + str(i) + ".attn.qkv")
            layer_name_list.append("encoder." + str(i) + ".attn.proj")
            layer_name_list.append("encoder." + str(i) + ".mlp.fc1")
            layer_name_list.append("encoder." + str(i) + ".mlp.act")
            layer_name_list.append("encoder." + str(i) + ".mlp.fc2")

            
output_act_dict_pretrained ={} 
output_act_dict_finetuned = {} 
def get_print_fn(name,ispretrained):
        def print_act(module, input, output):
            if ispretrained:
                output_act_dict_pretrained[name] = output
            else:
                output_act_dict_finetuned[name] = output
        return print_act
for n,m in model_depth_main.named_modules():
    if n in layer_name_list:
        m.register_forward_hook(get_print_fn(n,ispretrained=1))
        
for n,m in model.named_modules():
    n=n.replace('module.','')
    if n in layer_name_list:
       
        m.register_forward_hook(get_print_fn(n,ispretrained=0))  
                     
import copy
n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
return_all_layers = args.output_adapter in ['dpt']
total_layers=13
args.sparsity_pen=0.000000125
...
args.max_grad_norm=60
max_norm=args.clip_grad
total_layers=13
args.fix_layer=-1
args.gradient_accumulation_steps =1
args.save_ckp=2000000
number_next_frame=20

args.act_diff_l0_pen_per_layer_temporal="2.5e-10,"*12+"2.5e-10"
args.act_diff_l0_pen_per_layer_task="2.5e-10,"*12+"2.5e-10"
...
for epoch in range(args.start_epoch, args.epochs):
    ...
    for step, x in enumerate(metric_logger.log_every(trainloader, print_freq, header)):
        ...
        for n, p in model.named_parameters():
                layer_idx = get_num_layer(n)
                
                if n not in depth_params:
                    print(" n not in depth_params")
                    embed()
                assert(n in depth_params)
                if layer_idx <= args.fix_layer:
                    
                    nonzero_params += 0
                    p.data.copy_(depth_params[n][0].data)
                elif "output_adapters" in n:
                    #print("decoder")
        ...
        with torch.no_grad():
            outputs_pretrained =model_depth_main(input_dict['image'])
        #task domain computation
        ...
        act_diff_L0_loss_t = [0]*total_layers
        
        for n, act in output_act_dict_pretrained.items():
            layer_ind = get_num_layer_for_vit(n)
            if layer_idx > args.fix_layer:
                
                if n!='':
                    if type(act) is tuple:
                    #L1
                        diff_sum_L1= torch.sum(torch.abs(act[0]-output_act_dict_finetuned[n][0]))
                        act_diff_L0_loss_t[layer_ind-1] += diff_sum_L1
                    
                    else:
                    #L1
                        diff_sum_L1= torch.sum(torch.abs(act-output_act_dict_finetuned[n]))
                        act_diff_L0_loss_t[layer_ind-1] += diff_sum_L1
        
        act_diff_l0_pen_per_layer = [float(b) for b in args.act_diff_l0_pen_per_layer_task.split(",")]
        assert len(act_diff_l0_pen_per_layer) == 13, "Layer size should be matched"
 
        loss_task=0
        for i in range(13):
                loss += act_diff_l0_pen_per_layer[i] * act_diff_L0_loss_t[i]
                loss_task+=act_diff_l0_pen_per_layer[i] * act_diff_L0_loss_t[i]
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            for n, p in model.named_parameters():
                    layer_idx = get_num_layer_for_vit(n)
                    if p.grad is None:
                        continue
                    if layer_idx <= args.fix_layer:
                        
                        depth_params[n][1].grad.zero_()
                    elif "output_adapters" in n:
        ...
        metric_logger.update(loss=loss_value)
        metric_logger.update(loss_task=loss_task.item())
        metric_logger.update(**task_loss_values)
    ...
    if epoch % args.eval_freq == 0:
            ...
            if (100*val_stats['mIoU'] )> 71.5:
                min_val_loss = val_stats['mIoU']
                model_to_save = (
