...
class MultiTaskDatasetFolder(VisionDataset):
    ...
    def __init__(
        ...
        super(MultiTaskDatasetFolder, self).__init__(root, transform=transform,
                                            target_transform=target_transform)
        self.tasks = tasks
        self.in_domain=task_in
        classes, class_to_idx = self._find_classes(os.path.join(self.root))
        ...
        samples = {
            task: make_dataset(os.path.join(self.root, f'{prefixes[task]}{task}'), {cls_name: i for i, cls_name in enumerate({task})}, extensions, is_valid_file)
    ...
    def _find_classes(self, dir: str) -> Tuple[List[str], Dict[str, int]]:
        ...
        """
        classes = [d.name for d in os.scandir(dir) if d.is_dir()]
        #classes=self.in_domain
        #
        classes.sort()
        
        #print(classes)
        class_to_idx = {cls_name: i for i, cls_name in enumerate(classes) if cls_name in self.in_domain}
        #print(class_to_idx)
        return classes, class_to_idx
...
class MultiTaskImageFolder(MultiTaskDatasetFolder):
    ...
    def __init__(
        ...
        super(MultiTaskImageFolder, self).__init__(root, tasks,task_in, loader, IMG_EXTENSIONS if is_valid_file is None else None,
                                          transform=transform,
                                          target_transform=target_transform,
                                          is_valid_file=is_valid_file,
                                          prefixes=prefixes,
