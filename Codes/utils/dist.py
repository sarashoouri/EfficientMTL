...
def init_distributed_mode(args):
    ...
    torch.distributed.init_process_group(backend=args.dist_backend, init_method=dist_url,
                                         world_size=int(os.environ['WORLD_SIZE']), rank=int(os.environ['LOCAL_RANK']))
...
def collect_results_cpu(result_part, size, tmpdir=None):
    ...
    if tmpdir is None:
        ...
        dir_tensor = torch.full((MAX_LEN, ),
                                ...
                                device='cuda')
        if rank == 0:
            os.makedirs('/nfs/turbo/coe-hunseok/sshoouri/Codes/tmp', exist_ok=True)
            tmpdir = tempfile.mkdtemp(dir='/nfs/turbo/coe-hunseok/sshoouri/Codes/tmp')
            tmpdir = torch.tensor(
