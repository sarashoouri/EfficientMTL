...
args.finetune='./nyu/seg/step1/checkpoint-best.pth'
args.input_size=416
...
args.batch_size=20
args.lr=0.0001*(args.batch_size/256)
...
#os.environ['MASTER_PORT'] = '52689'

#dist_url = "tcp://%s:%s" % (os.environ['MASTER_ADDR'], os.environ['MASTER_PORT'])

#torch.distributed.init_process_group(backend=dist_backend, init_method=dist_url,world_size=world_size, rank=local_rank)
...
#args.in_domains =['rgb' ,'rgb_21' ,'rgb_22', 'rgb_23' ,'rgb_19' ,'rgb_18', 'rgb_17']
args.in_domains =['rgb' ,'rgb_21' ,'rgb_22' ,'rgb_19', 'rgb_18']
...
DOMAIN_CONF['rgb_18']=DOMAIN_CONF['rgb']
DOMAIN_CONF['rgb_19']=DOMAIN_CONF['rgb']
DOMAIN_CONF['rgb_21']=DOMAIN_CONF['rgb']
DOMAIN_CONF['rgb_22']=DOMAIN_CONF['rgb']
...
for i in range(0,12): 
            ...
            layer_name_list.append("encoder." + str(i) + ".attn.proj_drop")
...
args.act_diff_l0_pen_per_layer="40e-10,"*7+"40e-10,"*5+"40e-10"
args.save_ckpt_freq=500000
args.eval_freq=5
...
for epoch in range(args.start_epoch, args.epochs):
    ...
    loss1=0
    loss2=0
    for step, (x, _) in enumerate(metric_logger.log_every(data_loader_train, print_freq, header)):
        # assign learning rate & weight decay for each step
        it = start_steps + step  # global training iteration
        if lr_schedule_values is not None or wd_schedule_values is not None:
            for i, param_group in enumerate(optimizer.param_groups):
        ...
        while number_next_frame==20:
            number_next_frame=20+torch.randint(-2, 2, (1,))  
        with torch.no_grad():
            outputs_pretrained = model(tasks_dict['rgb_'+str(number_next_frame[0].numpy())]) 
            before_res=copy.deepcopy(output_act_dict_finetuned)
        ...
        with torch.cuda.amp.autocast(enabled=args.fp16):
            preds = model(input_dict['rgb'], return_all_layers=return_all_layers)
            seg_pred, seg_gt = preds['semseg'], tasks_dict['semseg']
            loss = criterion(seg_pred, seg_gt)
        loss_value = loss.item()
        
        
        act_diff_L0_loss = [0]*total_layers

        for n, act in output_act_dict_finetuned.items():
            layer_ind = get_num_layer(n)
            if n!='':
                if type(act) is tuple:
                    #t-1 _ L0
                    #diff_sum=torch.sum(torch.sigmoid(2*(act[0]-before_res[n][0]-1)))
                    #act_diff_L0_loss[layer_ind] += diff_sum
                    #t-1 _ L1
                    diff_sum_L1= torch.sum(torch.abs(act[0]-before_res[n][0]))
                    act_diff_L0_loss[layer_ind] += diff_sum_L1
                    
                  
                else:
                    #t-1 _ L0
                    #diff_sum=torch.sum(torch.sigmoid(2*(act-before_res[n]-1)))
                    #act_diff_L0_loss[layer_ind] += diff_sum
                    #t-1 _ L1
                    diff_sum_L1= torch.sum(torch.abs(act-before_res[n]))
                    act_diff_L0_loss[layer_ind] += diff_sum_L1
                          
        act_diff_l0_pen_per_layer = [float(b) for b in args.act_diff_l0_pen_per_layer.split(",")]
        assert len(act_diff_l0_pen_per_layer) == 13, "Layer size should be matched"
        loss2=loss_value
        loss1=0
        for i in range(13):
                loss += act_diff_l0_pen_per_layer[i] * act_diff_L0_loss[i]
                loss1+=act_diff_l0_pen_per_layer[i] * act_diff_L0_loss[i]
        ...
        # Metrics and logging
        metric_logger.update(loss2=loss2)
        
        metric_logger.update(loss=loss.item())
        metric_logger.update(loss_spare=loss1.item())
        if args.fp16:
            metric_logger.update(loss_scale=loss_scale_value)
        min_lr = 10.
        max_lr = 0.
