...
from .output_adapter_utils import (ConvNeXtBlock, Interpolate,
                                   make_fusion_block, make_scratch,  make_scratch_base)
...
class DPTOutputAdapter_base(nn.Module):
    """DPT output adapter.
    ...
    def __init__(self,
        ...
        self.scratch = make_scratch_base(layer_dims, feature_dim, groups=1, expand=False)
    ...
    def forward(self, encoder_tokens: List[torch.Tensor], input_info: Dict):
        ...
        # Fuse layers using refinement stages
        path_4 = self.scratch.refinenet2(layers[1])
        path_3 = self.scratch.refinenet1(path_4, layers[0])
        #path_2 = self.scratch.refinenet2(path_3, layers[1])
        #path_1 = self.scratch.refinenet1(path_2, layers[0])

        # Output head
        out = self.head(path_3)
