...
def get_diff_act_tensor_sparsity(x,x_pretrained,threshold):
    with torch.no_grad():
        nonzero = 0
        total = 0
        diff_act = x.data - x_pretrained.data 
        nonzero = torch.sum((torch.abs(diff_act) > threshold))
        total = diff_act.numel()
        return 1.0 * nonzero / total
    
def act_tensor_diff_per_batch(x,x_pretrained, threshold=0.15):
    with torch.no_grad():
            ...
            for batch_idx in range(num_batch):
                ...
                diff_act = x[batch_idx].data - x_pretrained[batch_idx].data
                diff_act[torch.abs(diff_act)<threshold] = 0.
                x.data[batch_idx] = x_pretrained.data[batch_idx] + diff_act
                #print(get_diff_act_tensor_sparsity(x[batch_idx],x_pretrained[batch_idx],threshold))
...
class Mlp(nn.Module):
    ...
    def forward(self, x, layer_number, pretrained_outputs = None, threshold=0.15):
        x = self.fc1(x)
        if pretrained_outputs is not None:
            layer_name = "encoder." +str(layer_number)+".mlp.fc1"
            th=threshold[int(layer_number)]
            x= act_tensor_diff_per_batch(x , pretrained_outputs[layer_name],th)
        x = self.act(x)
        # x = self.drop(x)
        ...
        if pretrained_outputs is not None:
            layer_name = "encoder." +str(layer_number)+".mlp.fc2"
...
class Attention(nn.Module):
    ...
    def forward(self, x,  layer_number, pretrained_outputs = None, threshold=0.15):
        ...
        if pretrained_outputs is not None:
            layer_name = "encoder." +str(layer_number)+".attn.qkv"
            th=threshold[int(layer_number)]
            qkv_2= act_tensor_diff_per_batch(qkv_2 , pretrained_outputs[layer_name],th)
        
        #qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        
        
        
        qkv = qkv_2.reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        ...
        if pretrained_outputs is not None:
            layer_name = "encoder." + str(layer_number)+".attn.proj"
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, layer_number=0):
        super().__init__()
    ...
    def forward(self, x, pretrained_outputs = None, threshold=0.15):
        x = x + self.drop_path(self.attn(self.norm1(x),self.layer_number, pretrained_outputs,threshold))
        x = x + self.drop_path(self.mlp(self.norm2(x),self.layer_number, pretrained_outputs,threshold))
        return x
