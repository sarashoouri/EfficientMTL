...
from .multimae_utils import Block, trunc_normal_, Block2, Block_mask_manu, Block_gumble_mask, Block_gumble_mask_one, Block_VIT_SLIM, Block_HiLo_attention, Block_HiLo_attention_7, Block_HiLo_attention_9
...
# MultiMAE module in HiLo attention
#alpha 0.5

class MultiMAE_HiLo_attention(nn.Module):
    """MultiMAE: Multi-task Multi-modal Masked Autoencoder
    This module performs masking in its forward pass.
    The MultiViT module defined below inherits from this module and performs a regular forward pass,
    and should be used instead for downstream tasks
    ...
    def __init__(self,
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        self.encoder = nn.Sequential(*[
            Block_HiLo_attention(dim=dim_tokens, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                  drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer)
            for i in range(depth)
        ])
...
class MultiMAE_HiLo_attention_7(nn.Module):
    """MultiMAE: Multi-task Multi-modal Masked Autoencoder
    This module performs masking in its forward pass.
    The MultiViT module defined below inherits from this module and performs a regular forward pass,
    and should be used instead for downstream tasks
    ...
    def __init__(self,
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        self.encoder = nn.Sequential(*[
            Block_HiLo_attention_7(dim=dim_tokens, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                  drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer)
            for i in range(depth)
        ])
...
class MultiMAE_HiLo_attention_9(nn.Module):
    """MultiMAE: Multi-task Multi-modal Masked Autoencoder
    This module performs masking in its forward pass.
    The MultiViT module defined below inherits from this module and performs a regular forward pass,
    and should be used instead for downstream tasks
    ...
    def __init__(self,
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        self.encoder = nn.Sequential(*[
            Block_HiLo_attention_9(dim=dim_tokens, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                  drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer)
            for i in range(depth)
        ])
...
#alpha 0.5
class MultiViT_HiLo_attention(MultiMAE_HiLo_attention):
    """MultiViT: Multi-modal Vision Transformer
    This is MultiMAE without masking and with a simplified / faster forward pass
...
@register_model
def multivit_base_HiLo_attention(
        input_adapters: Dict[str, nn.Module],
        output_adapters: Optional[Dict[str, nn.Module]],
        **kwargs):
    model = MultiViT_HiLo_attention(
        input_adapters=input_adapters,
        output_adapters=output_adapters,
        dim_tokens=768,
        depth=12,
...
#alpha 0.7
class MultiViT_HiLo_attention_7(MultiMAE_HiLo_attention_7):
    """MultiViT: Multi-modal Vision Transformer
    This is MultiMAE without masking and with a simplified / faster forward pass
...
@register_model
def multivit_base_HiLo_attention_7(
        input_adapters: Dict[str, nn.Module],
        output_adapters: Optional[Dict[str, nn.Module]],
        **kwargs):
    model = MultiViT_HiLo_attention_7(
        input_adapters=input_adapters,
        output_adapters=output_adapters,
        dim_tokens=768,
        depth=12,
...
#alpha 0.9
class MultiViT_HiLo_attention_9(MultiMAE_HiLo_attention_9):
    """MultiViT: Multi-modal Vision Transformer
    This is MultiMAE without masking and with a simplified / faster forward pass
...
@register_model
def multivit_base_HiLo_attention_9(
        input_adapters: Dict[str, nn.Module],
        output_adapters: Optional[Dict[str, nn.Module]],
        **kwargs):
    model = MultiViT_HiLo_attention_9(
        input_adapters=input_adapters,
        output_adapters=output_adapters,
        dim_tokens=768,
        depth=12,
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        #for adapter in input_adapters.values():
         #   adapter.init(embed_dim = embed_dim)
        self.input_adapters = nn.ModuleDict(input_adapters)
